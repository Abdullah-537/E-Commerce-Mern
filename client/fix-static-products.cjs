...
content = content.replace(/(<h3>Top Electronics<\/h3>[\s\S]*?<div className="swiper-wrapper">)[\s\S]*?(<\/div>\s*<\/div>\s*<div className="swiper-nav")/g, "$1\n" + productCardJSX + "\n$2");

// Also check if there are other static grids
// For instance "Explore new arrivals" or "Best Offers". They might not be swipers, but standard rows.
// Let's print out if it found Top Electronics
if (content.includes("Top Electronics")) {
   console.log("Top Electronics found and replaced");
}
