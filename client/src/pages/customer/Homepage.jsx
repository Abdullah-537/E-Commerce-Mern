...
export default function Homepage() {
  ...
  useEffect(() => {
    ...
    const initSwipers = () => {
      ...
      if (window.Swiper && swiperContainers.length > 0) {
        swiperContainers.forEach(container => {
          ...
          let config = {};
          try {
            config = JSON.parse(swiperEl.getAttribute('data-swiper') || '{}');
          } catch(e) {}
    ...
    const timeout = setTimeout(initSwipers, 100);
        ...
        <div className="container-small">
          <div className="scrollbar">
            <div className="d-flex justify-content-between">
              {categories.map(cat => {
                const iconMap = {
                  'books': { icon: 'uil uil-book-open', bg: 'bg-info-subtle', color: 'text-info' },
                  ...
                  'fashion': { icon: 'uil uil-watch-alt', bg: 'bg-warning-subtle', color: 'text-warning' },
                  'home': { icon: 'uil uil-estate', bg: 'bg-primary-subtle', color: 'text-primary' },
                  'sports': { icon: 'uil uil-football', bg: 'bg-secondary-subtle', color: 'text-secondary' },
                ...
                const match = Object.entries(iconMap).find(([k]) => cat.name.toLowerCase().includes(k));
                const { icon, bg, color } = match?.[1] || { icon: 'uil uil-apps', bg: '', color: '' };
                return (
