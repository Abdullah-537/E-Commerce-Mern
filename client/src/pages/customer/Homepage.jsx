...
function ProductCard({ product, showVendor = true }) {
  return (
        ...
        <div>
          ...
          <p className="fs-9">
            ...
            <span className="text-body-quaternary fw-semibold ms-1">
              ({product.reviewCount || Math.floor(Math.random() * 80 + 10)} people rated)
            </span>
          </p>
        </div>
        <div>
          ...
          <p className="text-body-tertiary fw-semibold fs-9 lh-1 mb-0">
            {product.salePrice && (
              <span className="text-success me-2">{Math.round((1 - product.salePrice / product.price) * 100)}% off</span>
            )}
